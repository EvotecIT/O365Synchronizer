<p align="center">
  ...
  <a href="https://www.powershellgallery.com/packages/O365Synchronizer"><img src="https://img.shields.io/powershellgallery/v/O365Synchronizer.svg?label=powershell%20gallery%20preview&colorB=yellow&include_prereleases"></a>
...
# preserve the older behavior that uses the Exchange mail alias as Nickname
Sync-O365PersonalContact -UserId 'przemyslaw.klys@test.pl' -NicknameSource MailNickname -Verbose

# tag synchronized contacts with categories
Sync-O365PersonalContact -UserId 'przemyslaw.klys@test.pl' -MemberTypes 'Member' -Category 'Friends', 'Work' -Verbose

# clear categories assigned by sync
Sync-O365PersonalContact -UserId 'przemyslaw.klys@test.pl' -MemberTypes 'Member' -Category @() -Verbose
```

Synchronized contacts use the GAL display name for the Outlook `Nickname` field by default. Existing managed contacts are corrected during their next sync. Use `-NicknameSource MailNickname` only when the Exchange mail alias is the intended visible nickname.

#### License requirements & external users

By default, users without assigned licenses are skipped. If you need to include all unlicensed users, use `-DoNotRequireAssignedLicenses`.
If you only want to include external users without licenses, use `-IncludeExternalUsers` with one or both values:
- `Guest` (UserType = Guest)
- `ExtUPN` (UserPrincipalName contains `#EXT#`)

```powershell
# include only external users without licenses (guests and #EXT# UPNs)
Sync-O365PersonalContact -UserId 'przemyslaw.klys@test.pl' -MemberTypes 'Member', 'Guest' -IncludeExternalUsers 'Guest', 'ExtUPN' -Verbose

# opt-in, best-effort Graph fallback only
Sync-O365PersonalContact -UserId 'przemyslaw.klys@test.pl' -MemberTypes 'Member' -ExcludeHiddenFromAddressList -HiddenAddressListSource Graph -Verbose

# users stay in scope when Graph returns showInAddressList as null, omits it, or does not expose it reliably

# recommended authoritative Exchange-backed filtering
Sync-O365PersonalContact -UserId 'przemyslaw.klys@test.pl' -MemberTypes 'Member', 'Contact' -ExcludeHiddenFromAddressList -HiddenAddressListSource Exchange -Verbose
```

Microsoft's Graph user documentation marks `showInAddressList` as "Do not use in Microsoft Graph". Because of that, `-HiddenAddressListSource Graph` is intentionally documented as an opt-in compatibility fallback only.
Use `-HiddenAddressListSource Graph` only when you want to stay on the existing Graph-only auth model and best-effort filtering is acceptable.
Use `-HiddenAddressListSource Exchange` when you want the recommended authoritative signal from Exchange Online and can provide an active Exchange session.

```powershell
# Exchange-backed hidden-address filtering requires an Exchange Online session
...
#### Homonyms and duplicate display names

By default, organization contact synchronization now generates a unique internal Exchange `Name` for each contact, which avoids creation failures for homonyms.

If you also want the visible `DisplayName` in Exchange/GAL to be unique, enable `-EnsureUniqueDisplayName`.
This keeps the first contact as-is and appends numeric suffixes such as `2`, `3`, and `4` for later duplicates.

```powershell
# keep visible names unique for homonyms
Sync-O365Contact -SourceObjects $UsersToSync -Domains 'evotec.pl','gmail.com' -EnsureUniqueDisplayName -Verbose -WhatIf
```
...
#### OData prefilter & extension attributes

```powershell
# prefilter users in Microsoft Graph before downloading all users
Sync-O365PersonalContact -UserId 'test@evotec.pl' -MemberTypes 'Member' -PassThru {
...
# filter locally by extension attribute after retrieval
Sync-O365PersonalContact -UserId 'test@evotec.pl' -MemberTypes 'Member' -PassThru {
...
# folder removal requires the folder to be empty; use -All if you want to remove every contact in that folder
# this is useful to clear current user contacts (if you have some)
